...
> 方法说明：全部内容通过与 WorkBuddy 里的 AI 助理连续对话完成——让它读本机清单、逆向客户端逻辑、写下载脚本、做统计分析。本质是「用 WorkBuddy 拆解 WorkBuddy」，没有任何内部渠道。

数据基准：**2026-08-19**，基于 425 个已安装插件包的真实文件系统扫描（421 市场公开批量 + 4 位无公开包专家 GUI 召唤）。
...
| 已装到本地 | 425（421 批量 + 4 位内部 GUI 召唤） |
| 无公开包专家中外部可召唤 | 仅 4 位（AICoding / 庄老师 / 送报报 / MigraQ），其余 15 位为内部专属（`internal_only` 或服务端限制） |
...
> 「440 个专家」也是错觉：19 位无公开包专家里只有 4 位外部账号能召唤，其余 15 位是腾讯内部专属。
...
│   ├── 06-安装机制逆向.md      （召唤 = 真安装，公开 COS 源，visibility 可见性机制）
...
- **19 位无公开包专家里 15 位是内部专属、外部召唤不了**——manifest 的 `visibility` 字段：12 位 `internal_only`（GUI 不展示）+ 3 位受 `isOPC`/服务端白名单限制；真正外部可召唤的只有 AICoding 架构专家团、庄老师、送报报、MigraQ 4 位。
- **manifest 的 `plugin` 字段并不可靠**——送报报声明 `industry-service-experts`，实际安装目录是 `report-distribution-agent`。
