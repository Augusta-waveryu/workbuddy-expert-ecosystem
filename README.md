...
数据基准：**2026-08-19**，基于 425 个已安装插件包的真实文件系统扫描（421 市场公开批量 + 4 位内部专家 GUI 召唤）。
...
| 内部专家中外部可召唤 | 仅 4 位（AICoding / 庄老师 / 送报报 / MigraQ），其余 15 位 `internal_only` 或服务端限制 |
...
> 「440 个专家」也是错觉：19 位内部专家里只有 4 位外部账号能召唤。
...
- **19 位内部专家里 15 位外部召唤不了**——manifest 的 `visibility` 字段：12 位 `internal_only`（GUI 不展示）+ 3 位受 `isOPC`/服务端白名单限制；真正外部可召唤的只有 AICoding 架构专家团、庄老师、送报报、MigraQ 4 位。
